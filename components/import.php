...
if (isset($_FILES['html_file']) && $_FILES['html_file']['error'] === UPLOAD_ERR_OK) {
    ...
    foreach ($entries as $entry) {
        if ($entry->getAttribute('class') !== 'entry') continue;
        $h2 = $entry->getElementsByTagName('h2');
        $divs = $entry->getElementsByTagName('div');
        $title = $h2->length > 0 ? trim($h2->item(0)->textContent) : '';
        $content = '';
        foreach ($divs as $div) {
            if (strpos($div->getAttribute('class'), 'meta') === false) {
                $content = trim($div->textContent);
                break;
            }
        }
        if ($content !== '') {
            $input[] = ['title' => $title, 'content' => $content, 'diary' => 'HTML import'];
        }
    }
    if (empty($input)) {
        echo json_encode(['success' => false, 'error' => 'Nepodarilo sa načítať zápisy z HTML súboru.']);
        exit;
    }
} else {
    // JSON import
    ...
    if (!is_array($input) || empty($input)) {
        echo json_encode(['success' => false, 'error' => 'Neplatný formát súboru. Podporovaný je JSON a HTML.']);
        exit;
    }
}
